...
public class PartEnergyInterface
	...
	public Object getClientGuiElement(EntityPlayer player)
	{
		return new GuiEnergyInterface((ContainerEnergyInterface) getServerGuiElement(player),
				this, player.world, this.getX(),this.getY(),this.getZ(),this.getSide().getFacing(), player );
	}
	@Override
	...
	public int InjectEnergy(IGridNode node,FluidStack resource, boolean doFill) {
		...
		IAEEnergyStack returnAmount;
		if (doFill) { // Modulation
			returnAmount = storage.getInventory(this.getChannel()).injectItems(
					this.getChannel().createStack(resource),
	...
	public int ExtractEnergy(IGridNode node,FluidStack resource, boolean doFill) {
		...
		if (doFill) {
			notRemoved = (IAEEnergyStack)storage.getInventory(getChannel()).extractItems(getChannel().createStack(resource), Actionable.MODULATE, new MachineSource(this));
		}
		else
		{
			notRemoved = (IAEEnergyStack)storage.getInventory(getChannel()).extractItems( getChannel().createStack(resource), Actionable.SIMULATE, new MachineSource(this));
		}
