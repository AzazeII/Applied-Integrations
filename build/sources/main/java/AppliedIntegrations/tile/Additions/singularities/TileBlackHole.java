...
public class TileBlackHole extends TileEntity implements ITickable, ISingularity {
    ...
    private void modulateBlockGravity() {
        ...
        if(handler.hasTimePassed(world, 1)) {
            ...
            for (int x = -(int) range; x < range; x++) {
                ...
                for (int y = -(int) range; y < range; y++) {
                    ...
                    for (int z = -(int) range; z < range; z++) {
                        ...
                        Vec3d A = new Vec3d(blockPos);
                        // Vector representation of this block pos
                        Vec3d B = new Vec3d(pos);
                        ...
                        if (b == world.getBlockState(pos).getBlock() && !crossesDeadEndRadius(A, B))
                            continue;

                        // Check if block is air
                        if (b instanceof BlockAir && !crossesDeadEndRadius(A, B))
                            continue;
                        ...
                        if (b instanceof BlockSingularity && !crossesDeadEndRadius(A, B))
                            continue;

                        // Check if block is ME pylon
                        if (b instanceof BlockMEPylon && !crossesDeadEndRadius(A, B)) {
            ...
            /** Giant perfomance break:
            // Sort list, with classic bubble sorting algorithm
            int n = toBreak.size();
            for (int i = 0; i < n-1; i++) {
                for (int j = 0; j < n - i - 1; j++) {

                    // Get vector representation of positions
                    Vec3d vectorA = new Vec3d(toBreak.get(j));
                    Vec3d vectorB = new Vec3d(toBreak.get(j+1));

                    // Get vector representation of pos of black hole center
                    Vec3d vectorCenter = new Vec3d(pos);

                    // Check if vector distance of A is greater than B
                    if (vectorA.distanceTo(vectorCenter) > vectorB.distanceTo(vectorCenter)) {
                        // Create buffer
                        BlockPos buffer = toBreak.get(j);
                        // Swap elements
                        toBreak.set(j, toBreak.get(j+1));
                        toBreak.set(j+1, buffer);
                    }
                }
            }*/
            ...
            for (BlockPos b : toBreak) {
                ...
                // Check if range to this block is greater than break range
                if (crossesDeadEndRadius(positionVectorA, positionVectorB))
                    // Skip this pos
                    continue;
    ...
    private boolean crossesDeadEndRadius(Vec3d a, Vec3d b) {
        // Check if distance not equal to black hole radius
        return a.distanceTo(b) > getBlackHoleRadius();
    }
