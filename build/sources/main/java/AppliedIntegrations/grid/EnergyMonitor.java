...
public class EnergyMonitor
        implements IMEEnergyMonitor, IMEMonitorHandlerReceiver<IAEEnergyStack>
{
    /**
     ...
     */
    protected IMEMonitor<IAEEnergyStack> fluidMonitor;
                                    ...
                                    final LiquidAIEnergy energyGas )
    {
        ...
        IAEEnergyStack fluidRequest = Utils.ConvertToAEFluidStack( energyGas, amount );

        // Inject fluid
        IAEEnergyStack fluidRejected = this.fluidMonitor.injectItems( fluidRequest, mode, source );
    ...
    protected void updateCacheToMatchNetwork()
    {
        ...
        if( this.energyGrid.isNetworkPowered() )
        {
            ...
            IItemList<IAEEnergyStack> fluidStackList = this.fluidMonitor.getStorageList();

            // Loop over all fluids
            for( IAEEnergyStack fluidStack : fluidStackList )
            {
                // Ensure the fluid is an energy 
                if( !( fluidStack.getEnergy() instanceof LiquidAIEnergy) )
                {
                    // Not an energy .
                    continue;
                }
    ...
    public long extractEnergy(final LiquidAIEnergy energy, final long amount, final Actionable mode, final BaseActionSource source, final boolean powered )
    {
        ...
        // Create the fluid request
        IAEEnergyStack fluidRequest = Utils.ConvertToAEFluidStack(energy,amount);

        // Attempt the extraction
        IAEEnergyStack fluidReceived = this.fluidMonitor.extractItems( fluidRequest, mode, source );
    ...
    @Override
    public void postChange(IBaseMonitor<IAEEnergyStack> iBaseMonitor, Iterable<IAEEnergyStack> fluidChanges, IActionSource iActionSource) {
        ...
        for( IAEEnergyStack change : fluidChanges )
        {
            // Is the change an energy ?
            if( ( change.getEnergy() instanceof LiquidAIEnergy) )
            {
                // Get the energy
                LiquidAIEnergy energy = ( (LiquidAIEnergy)change.getEnergy() ).getEnergy();
