...
public class PartEnergyExport extends AIOPart {
	...
	public int ExtractEnergy(FluidStack resource, Actionable mode) {
		...
		if (mode == MODULATE) {
			notRemoved = (IAEFluidStack)storage.getInventory(getChannel()).extractItems(
					getChannel().createStack(resource), MODULATE, new MachineSource(this));
		}
		else
		{
			notRemoved = (IAEFluidStack)storage.getInventory(getChannel()).extractItems(getChannel().createStack(resource), SIMULATE, new MachineSource(this));
		}
