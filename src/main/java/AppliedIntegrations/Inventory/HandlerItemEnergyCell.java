        ...
        implements IMEInventoryHandler<IAEFluidStack>
{
    ...
    public HandlerItemEnergyCell(final ItemStack storageStack, final ISaveProvider saveProvider )
    {
        ...
        this.totalTypes = ( (EnergyStorageCell)ItemEnum.ENEGYSTORAGE.getItem() ).maxTypes( storageStack );

        // Get the total bytes we can hold
        this.totalBytes = ( (EnergyStorageCell)ItemEnum.ENEGYSTORAGE.getItem() ).maxStorage( storageStack );
    ...
    public boolean canAccept( final IAEFluidStack input )
    {
        ...
        if (!(input.getFluid() instanceof LiquidAIEnergy))
            return false;
    ...
    public IItemList<IAEFluidStack> getAvailableItems( final IItemList<IAEFluidStack> availableList )
    {
        for( IEnergyStack EnergyStack : this.storedEnergy )
        {
            ...
            availableList.add(AEApi.instance().storage().createFluidStack(new FluidStack(Energy,(int)EnergyStack.getStackSize())));
    ...
    public IAEFluidStack injectItems( final IAEFluidStack input, final Actionable mode, final BaseActionSource src )
    {
        ...
        if( ( input.getFluid() == null ) || !( input.getFluid() instanceof LiquidAIEnergy) )
        {
            // Invalid fluid
            return input.copy();
        }
        ...
        // Get the Energy
        LiquidAIEnergy EnergyEnergy = (LiquidAIEnergy)input.getFluid();
